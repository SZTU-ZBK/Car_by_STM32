...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
    ...
    if(huart->Instance == UART4)                /* 濡傛灉鏄覆鍙?锛岃繘琛屼覆鍙? MSP鍒濆鍖?*/
    {
		__HAL_RCC_UART4_CLK_ENABLE();
		__HAL_RCC_GPIOC_CLK_ENABLE();//浣胯兘鏃堕挓
		...
		HAL_GPIO_Init(GPIOC, &gpio_init_struct);//GPIO澶嶇敤鍒濆鍖?
		HAL_NVIC_SetPriority(UART4_IRQn,2,3);//璁剧疆涓柇浼樺厛绾?
		HAL_NVIC_EnableIRQ(UART4_IRQn);//浣胯兘涓柇
	}
